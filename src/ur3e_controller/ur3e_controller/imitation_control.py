...
class ConditionalDiffusionModel(nn.Module):
    def __init__(self, action_dim=7, output_dim=10,sensor_dim=7,depth_features_dim=512, hidden_dim=256, num_layers=2):
        ...
        self.depth_encoder = SimpleCNN(1, (150,350), depth_features_dim)
...
class ImitationControl(Node):
    def __init__(self) -> None:
        ...
        check_point=torch.load('/home/siddiquieu1/ros2/flow_matching_real_world_data_epoch_1500.pt', map_location=self.device)
    ...
    @torch.no_grad()
    # def _run_model(self, depth_images, non_visual_obs, num_predicted_actions=10, action_sequence_length=20, action_dim=7, num_denoising_steps=100, device=None  ) -> List[List[float]]:
    def _run_model(self, observations: Sequence[Observation]) -> List[List[float]]:
        # _ = observations
