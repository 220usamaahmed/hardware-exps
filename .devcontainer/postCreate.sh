...
GRIPPER_DIR="${SRC_PATH}/ecpmi_gripper"
...
if [[ ! -d "${GRIPPER_DIR}" ]]; then
	echo "Cloning ECPMI gripper repository..."
	git clone git@github.com:RoboticManipulation/ecpmi_gripper.git "${GRIPPER_DIR}"
else
	echo "ECPMI gripper repository already present."
fi
...
export ZED_CAMERA_WS=$SRC_PATH/zed_ws
...
sudo chown -R "${USERNAME:-$(id -un)}":"${USERNAME:-$(id -un)}" /usr/local/zed
...
cd ~/ros2/src/zed_ws/src/zed-ros2-wrapper
git fetch --all --tags
